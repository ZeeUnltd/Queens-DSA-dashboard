...
type TransactionExportDialogProps = {
  ...
  error: string | null
  onClose: () => void
  onExport: (values: { startDate: string; endDate: string; accountNumber: string; amount: string; pageNumber: number; pageSize: number }) => void
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !isExporting) onClose() }}>
      <section className="w-full max-w-[520px] rounded-[24px] bg-white p-6 shadow-2xl sm:p-8" role="dialog" aria-modal="true" aria-labelledby="transaction-export-title">
        <div className="flex items-start justify-between gap-4">
          <div><h2 className="text-2xl font-bold text-qm-ink" id="transaction-export-title">Export transactions</h2><p className="mt-1 text-sm text-qm-muted">Choose the transactions to include in the CSV.</p></div>
        ...
        <form className="mt-6 grid gap-4" onSubmit={(event) => { event.preventDefault(); onExport({ startDate: start, endDate: end, accountNumber: account.trim(), amount, pageNumber, pageSize }) }}>
          ...
          <div className="mt-2 flex justify-end gap-3"><Button className="rounded-full px-5" variant="outline" type="button" disabled={isExporting} onClick={onClose}>Cancel</Button><Button className="rounded-full bg-qm-brand px-5 hover:bg-qm-brand-hover" type="submit" disabled={isExporting}>{isExporting ? 'Exporting...' : 'Download CSV'}</Button></div>
        </form>
      </section>
    </div>
  )
