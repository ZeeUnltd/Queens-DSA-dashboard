...
function DsaDirectoryView() {
  ...
  const agents = filteredAgents.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE)
  ...
  return (
    <section>
      ...
      <div className="overflow-x-auto rounded-xl border border-[#eeeeee]"><table className="w-full min-w-[820px] text-xs"><thead><tr className="border-b border-[#f1f1f1] text-left text-[10px] text-qm-muted"><th className="px-4 py-4">DSA Full Name</th><th>Account Opened</th><th>Inflow</th><th>Outflow</th><th>Balance</th><th>Active Mandates</th><th /></tr></thead><tbody>{agents.map((agent) => <tr className="border-b border-[#f1f1f1] last:border-0" key={agent.id}><td className="px-4 py-3"><span className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#fff6f6] text-[10px] font-bold text-qm-brand">{agent.initials}</span><strong>{agent.name}</strong></span></td><td>{agent.accountsOpened}</td><td>{agent.inflow}</td><td>{agent.outflow}</td><td>{formatCurrency(agent.balance)}</td><td>{agent.activeMandates}</td><td className="pr-4 text-right"><button className="font-semibold text-qm-brand hover:underline" type="button" onClick={() => setSelectedId(agent.id)}>View Details</button></td></tr>)}</tbody></table><nav className="flex items-center justify-between border-t border-[#f1f1f1] px-4 py-3 text-xs text-qm-muted" aria-label="DSA pagination"><button className="flex items-center gap-1 rounded-md border border-[#eeeeee] px-3 py-2 disabled:opacity-40" type="button" disabled={currentPage === 1} onClick={() => setPage((value) => value - 1)}><ChevronLeft className="h-3.5 w-3.5" />Previous</button><span>Page {currentPage} of {pageCount}</span><button className="flex items-center gap-1 rounded-md border border-[#eeeeee] px-3 py-2 disabled:opacity-40" type="button" disabled={currentPage === pageCount} onClick={() => setPage((value) => value + 1)}>Next<ChevronRight className="h-3.5 w-3.5" /></button></nav></div>
